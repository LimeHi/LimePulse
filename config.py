...
#
# ФИКС (было): раньше это были ДВА разных шага (отдельная "пустая"
# connectivity-проба на generate_204 + отдельно ipify), и TEST_URL_PRIMARY
# был URL-ом generate_204-типа (без JSON), нигде фактически не
# использовавшимся. generate_204/"пинг"-пробы вообще плохо подходят для
# наших реалий: они лишь подтверждают, что куда-то долетел пустой ответ
# 204, но НЕ доказывают, что трафик реально ушёл через прокси-сервер, а
# не был подделан транспарентным DPI/block-page — именно они и были
# убраны, ICMP тут никогда не использовался (aiohttp работает только
# поверх TCP через SOCKS5-инбаунд sing-box, ICMP через SOCKS5 в принципе
# не проксируется).
#
# ФИКС #2 (актуально): список провайдеров расширен и специально собран
# из сервисов на РАЗНОЙ инфраструктуре (свой bare-metal/anycast у ipify,
# независимая anycast-сеть у ipwho.is, geojs, seeip, myip.com — не всё на
# одном Cloudflare/AWS), чтобы блокировка/деградация одного хостинга или
# рейт-лимит одного сервиса не выводили из строя всю verify-цепочку.
# Один унифицированный список, TEST_URL_PRIMARY пробуется первым (но
# порядок дальше перемешивается на каждый прогон, см. _verify), при
# неудаче — перебор запасных.
...
TEST_URL_VERIFY_FALLBACKS = [
    # ipify — второй домен той же компании (IPv4/IPv6), но независимый DNS/host
    "https://api64.ipify.org?format=json",
    # ifconfig.co — своя инфраструктура, давно живёт, отдельный от ipify стек
    "https://ifconfig.co/json",
    # ipinfo.io — отдельный крупный провайдер, свой anycast
    "https://ipinfo.io/json",
    # ipwho.is (бывш. ipwhois.io) — независимая anycast-сеть, HTTPS,
    # без ключа API, 1000 запросов/сутки с IP — с запасом для чекера
    "https://ipwho.is/",
    # geojs.io — лёгкий отдельный сервис, минимальный JSON-ответ {"ip": "..."}
    "https://get.geojs.io/v1/ip.json",
    # seeip.org — ещё одна независимая пара (свои сервера, не шарит инфру
    # с перечисленными выше)
    "https://api.seeip.org?format=json",
    # api.myip.com — простой и старый сервис, ещё один независимый узел
    "https://api.myip.com",
]
